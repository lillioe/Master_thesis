...
def load_gene_data(nt_logits_path: Path) -> Dict[str, GeneData]:
    ...
    if missing:
        raise ValueError(f"nt_logits.tsv is missing columns: {sorted(missing)}")

    before = len(df)
    df = df.drop_duplicates(["gene_id", "candidate_index"])
    after = len(df)
    if after != before:
        print(f"[warning] dropped duplicate candidate rows: {before - after}")
...
class GeneAveragedDataset(Dataset):
    """
    One item = one gene
    item contains all valid transcript paths for that gene, 
    during training, the CRF loss is computed separately for each transcript
    path and then averaged within the gene before one optimizer update
    This changes the objective from:

        average over transcript examples

    to:

        average over genes of the within-gene average transcript loss
    """
    ...
    def __init__(
        ...
        by_gene: Dict[str, List[TranscriptExample]] = {}
        for ex in examples:
            if ex.gene_id not in gene_data:
                continue
            by_gene.setdefault(ex.gene_id, []).append(ex)

        self.examples_by_gene = by_gene
        self.gene_ids = sorted(by_gene)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        gene = self.gene_data[gene_id]
        examples = self.examples_by_gene[gene_id]

        labels = torch.stack(
            [
                build_labels_for_example(
                    gene=gene,
                    example=ex,
                    label_mode=self.label_mode,
                )
        ...
        return {
            "gene_id": gene_id,
            "transcript_ids": [ex.transcript_id for ex in examples],
            "candidate_indices": gene.candidate_indices,
            "site_logits": gene.site_logits,
            "candidate_type_ids": gene.candidate_type_ids,
            "labels": labels,  # (n_transcripts_for_gene, L)
            "n_transcripts": len(examples),
        }


def collate_gene_batch(items: List[dict]) -> dict:
    """
    Gene-averaged training uses batch_size=1.

    This avoids padding across genes. Each item still contains all transcript
    paths for that gene, so the model batch dimension is the number of
    transcript paths of the current gene.
    """
    if len(items) != 1:
        raise ValueError(
            "Gene-averaged training currently requires --batch-size 1. "
            "Each optimizer step should correspond to one gene."
        )
    return items[0]
...
def _gene_item_to_model_batch(item: dict, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Convert one gene item into a model batch
    Input item:
        site_logits: (L, 2)
        candidate_type_ids: (L,)
        labels: (M, L), where M is number of transcript paths for this gene

    Returned tensors:
        site_logits: (M, L, 2)
        candidate_type_ids: (M, L)
        attention_mask: (M, L)
        labels: (M, L)
    """
    ...
    site_logits = item["site_logits"].unsqueeze(0).expand(M, L, 2).to(device)
    candidate_type_ids = item["candidate_type_ids"].unsqueeze(0).expand(M, L).to(device)
    attention_mask = torch.ones((M, L), dtype=torch.bool, device=device)

    return site_logits, candidate_type_ids, attention_mask, labels


def train_one_epoch_gene_averaged(
    model: SplicePathCRFModel,
    loader: DataLoader,
    optimizer: torch.optim.Optimizer,
    device: torch.device,
    ...
    """
    One optimizer update per gene
    For each gene, the model sees all transcript paths for that gene as a
    batch. CRF loss returned by the model is the mean over those transcript
    paths. Each gene contributes one loss value and one optimizer
    update
    """
    model.train()
    ...
    for item in loader:
        site_logits, candidate_type_ids, attention_mask, labels = _gene_item_to_model_batch(
            item=item,
            device=device,
        )
    ...
    return total_loss / max(total_genes, 1)


@torch.no_grad()
def evaluate_loss_gene_averaged(
    model: SplicePathCRFModel,
    loader: DataLoader,
    device: torch.device,
) -> float:
    """
    Validation loss averaged by gene, not by transcript
    """
    model.eval()
...
def main() -> None:
    ...
    print("Loading transcript examples...")
    ...
    train_dataset = GeneAveragedDataset(
        examples=train_examples,
        gene_data=gene_data,
        label_mode=args.label_mode,
    )
    val_dataset = GeneAveragedDataset(
        examples=val_examples,
        gene_data=gene_data,
        label_mode=args.label_mode,
    )

    print(f"Train gene averag items: {len(train_dataset)}")
    print(f"Val gene averag items: {len(val_dataset)}")
    ...
    config_json = {
        ...
        "n_train_gene_items": len(train_dataset),
        "n_val_gene_items": len(val_dataset),
    }
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        train_loss = train_one_epoch_gene_averaged(
            model=model,
            loader=train_loader,
            optimizer=optimizer,
            device=device,
        ...
        val_loss = evaluate_loss_gene_averaged(
            model=model,
            loader=val_loader,
            device=device,
        )
        ...
        print(
            f"epoch={epoch:03d} "
            f"train_geneavg_loss={train_loss:.6f} "
            f"val_geneavg_loss={val_loss:.6f} "
        ...
        if args.patience and bad_epochs >= args.patience:
            print(f"Early stopping after {bad_epochs} epochs without val_loss improvement.")
            break

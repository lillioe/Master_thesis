...
class GeneWindowDataset(Dataset):
    """
    Loader stores all candidates for each gene, dataset decides which
    candidates are actually sent to the model.

    sampling_mode="first":
        old behavior after loading all candidates: take first N by position

    sampling_mode="class_aware":
        split candidates into positives/negatives, sample a small positive
        subset and fill the remaining slots with negatives. For training this is
        dynamic, for validation it is deterministic/fixed.
    """

    def __init__(
        self,
        examples: list[dict],
        max_candidates_per_gene: int | None = None,
        sampling_mode: str = "class_aware",
        is_train: bool = False,
        train_pos_min: int = 8,
        train_pos_max: int = 24,
        val_pos_target: int = 16,
        seed: int = 13,
    ):
        self.examples = examples
        self.max_candidates_per_gene = max_candidates_per_gene
        self.sampling_mode = sampling_mode
        self.is_train = is_train
        self.train_pos_min = train_pos_min
        self.train_pos_max = train_pos_max
        self.val_pos_target = val_pos_target
        self.seed = seed
        self.rng = random.Random(seed)

        if self.sampling_mode not in {"first", "class_aware"}:
            raise ValueError(f"Unknown sampling_mode: {self.sampling_mode}")

    def __len__(self) -> int:
        return len(self.examples)

    def _sample_indices(self, item: dict, idx: int) -> list[int]:
        n = len(item["windows"])

        if self.max_candidates_per_gene is None or n <= self.max_candidates_per_gene:
            return list(range(n))

        cap = self.max_candidates_per_gene

        if self.sampling_mode == "first":
            selected = list(range(cap))
            return sorted(selected, key=lambda i: item["pos"][i])

        labels = item["labels"]
        pos_idx = [i for i, y in enumerate(labels) if int(y) == 1]
        neg_idx = [i for i, y in enumerate(labels) if int(y) == 0]

        # Training changes every time __getitem__ is called
        # Validation is fixed per gene
        if self.is_train:
            rng = self.rng
            target_pos = rng.randint(self.train_pos_min, self.train_pos_max)
        else:
            rng = random.Random(self.seed + idx * 1_000_003)
            target_pos = self.val_pos_target

        target_pos = min(target_pos, cap)

        selected: list[int] = []

        if pos_idx and target_pos > 0:
            k_pos = min(target_pos, len(pos_idx))
            selected.extend(rng.sample(pos_idx, k_pos))

        remaining = cap - len(selected)

        if neg_idx and remaining > 0:
            k_neg = min(remaining, len(neg_idx))
            selected.extend(rng.sample(neg_idx, k_neg))

        # If a gene has too few negatives, fill remaining slots with positives
        # not already selected
        remaining = cap - len(selected)
        if remaining > 0:
            already = set(selected)
            extra_pool = [i for i in range(n) if i not in already]
            if extra_pool:
                selected.extend(rng.sample(extra_pool, min(remaining, len(extra_pool))))

        # Keep genomic order for the candidate Transformer
        return sorted(selected, key=lambda i: item["pos"][i])

    def __getitem__(self, idx: int) -> dict:
        item = self.examples[idx]
        selected = self._sample_indices(item, idx)

        windows = [item["windows"][i] for i in selected]
        labels = [item["labels"][i] for i in selected]
        pos = [item["pos"][i] for i in selected]
        candidate_types = [item["candidate_types"][i] for i in selected]
...
def load_gene_examples_from_tsv(
    ...
    """
    Load all candidate records for each selected gene
    Candidate capping/sampling happens in GeneWindowDataset.__getitem__.
    """
...
def count_labels_in_dataset(dataset: Dataset, num_classes: int) -> torch.Tensor:
    counts = torch.zeros(num_classes, dtype=torch.long)

    for i in range(len(dataset)):
        y = torch.tensor(dataset[i]["labels"], dtype=torch.long)
...
def main():
    ...
    parser.add_argument("--max-candidates-per-gene", type=int, default=128)
    parser.add_argument("--sampling-mode", choices=["first", "class_aware"], default="class_aware")
    parser.add_argument("--train-pos-min", type=int, default=8)
    parser.add_argument("--train-pos-max", type=int, default=24)
    parser.add_argument("--val-pos-target", type=int, default=16)
    ...
    val_counts_underlying = count_labels(val_examples, args.num_classes)

    print(f"Underlying train label counts: {train_counts.tolist()}")
    print(f"Underlying val label counts:   {val_counts_underlying.tolist()}")
    print(
        "Candidate sampling: "
        f"mode={args.sampling_mode}; "
        f"max_candidates_per_gene={args.max_candidates_per_gene}; "
        f"train_pos_range={args.train_pos_min}-{args.train_pos_max}; "
        f"val_pos_target={args.val_pos_target}"
    )

    train_dataset = GeneWindowDataset(
        train_examples,
        max_candidates_per_gene=args.max_candidates_per_gene,
        sampling_mode=args.sampling_mode,
        is_train=True,
        train_pos_min=args.train_pos_min,
        train_pos_max=args.train_pos_max,
        val_pos_target=args.val_pos_target,
        seed=args.seed,
    )

    val_dataset = GeneWindowDataset(
        val_examples,
        max_candidates_per_gene=args.max_candidates_per_gene,
        sampling_mode=args.sampling_mode,
        is_train=False,
    ...
    val_counts = count_labels_in_dataset(val_dataset, args.num_classes)
    print(f"Fixed sampled val label counts: {val_counts.tolist()}")
    ...
    if args.resume_from:
        ...
        if args.override_lr_on_resume:
            ...
            print("Overrode optimizer learning rates after resume:")
